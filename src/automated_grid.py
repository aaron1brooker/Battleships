...
class AutomatedGrid(PlayerGrid):
	...
	def auto_place_boat(self, boat: str):
		...
		while not has_placed:
			...
			if self.place_boat((rnd_x, rnd_y), boat, INVERT_DIRECTION[random.randint(1,4)]):
				has_placed = True
	...
	def auto_place_all(self):
		...
		players_boats = copy.deepcopy(self._player_boats) # Required as self._player_boats will change size
