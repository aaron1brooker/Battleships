...
from svc.exceptions import UserError
...
class GameSetup:
	...
	def __validate_and_parse_board(self) -> Tuple[bool, Dict]:
		...
		if (not (x or y)) or (not ((5 <= x <= 80) and (5 <= y <= 80))):
			...
			logging.warning("Board size is not in the valid range between 5 - 80")
			return False, {}
	...
	def get_game_config(self) -> Tuple[Dict[str, int], Dict[str, int]]:
		...
		raise UserError(f"Grid size is not valid. Board values: {self.__board}")
